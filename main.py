...
embed=discord.Embed(title="Commands:", description="Type =helpcat [category] for commands on the specific category.", color=0x00ffff)
...
economyembed=discord.Embed(title="Economy", description="Below are a list of commands that do something with economy. (Economy commands currently do not work yet.)", color=0x00ffff)
...
economyembed.set_author(name="Smokin' Goose", icon_url="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRBjrw3JnC6dABO3jnLvizGcpGQrn3bek812g&usqp=CAU")
...
miscembed.add_field(name="ping", value="Shows the latency of the bot.", inline=False)
...
randomembed.add_field(name="roll", value="Rolls a dice with a custom number of sides. =roll [# of sides]", inline=False)
economyembed.add_field(name="petgoose", value="Pet the Smokin Goose and gain some coins. Losing coins is impossible with this command. Alias: pet", inline=False)
economyembed.add_field(name="dailysalute", value="Salute the Smokin Goose and get your daily coins. Alias = salute", inline=False)
economyembed.add_field(name="feedgoose", value="Feed the Smokin Goose and gain some coins. You can also lose coins this way. Alias: feed", inline=False)
mathembed.add_field(name="add", value="Addition, =add [value1] [value2]", inline=False)
mathembed.add_field(name="sub", value="Subtraction of value 2 from value 1, =sub [value1] [value2]", inline=False)
mathembed.add_field(name="multi", value="Multiplication, =multi [value1] [value2]", inline=False)
mathembed.add_field(name="div", value="Division of value 1 by value 2 =div [value1] [value2]", inline=False)
mathembed.add_field(name="pwr", value="Value 1 to the power of value 2, =pwr [value1] [value2]", inline=False)
mathembed.add_field(name="root", value="The value 2 root of value 1, =root [value1] [value2]", inline=False)
...
embed.add_field(name="Economy", value="Commands that have to do with the bot's currency (incomplete)", inline=False)
...
@bot.command()
async def joineconomy(ctx):
  with open("users.json", "r") as f:
    data = json.load(f)

  data[str(ctx.author.id)] = {}
  data[str(ctx.author.id)]["coins"] = 100
  data[str(ctx.author.id)]["inventory"] = []
  data[str(ctx.author.id)]["job"] = "none"
  data[str(ctx.author.id)]["experience"] = 0

  with open("users.json", "w") as f:
    json.dump(data, f, indent=4)

  await ctx.reply("You have began your journey!")
...
async def owneronly(ctx):
    ...
    if ctx.message.author.id == yourID:
      ...
      await ctx.send('Access denied, only Hyperwolf can use this command.')
